// The SteamManager is designed to work with SteamworksNet.NET
...
//
// The SteamManager provides a base implementation of SteamworksNet.NET on which you can build upon.
...
public class SteamManager : MonoBehaviour {
	...
	protected virtual void Awake() {
		...
		if (!DllCheck.Test()) {
			Debug.LogError("[SteamworksNet.NET] DllCheck Test returned false, One or more of the SteamworksNet binaries seems to be the wrong version.", this);
		}
		...
		try {
			...
			Debug.LogError("[SteamworksNet.NET] Could not load [lib]steam_api.dll/so/dylib. It's likely not in the correct location. Refer to the README for more details.\n" + e, this);

			Application.Quit();
			return;
		}

		// Initializes the SteamworksNet API.
		// If this returns false then this indicates one of the following conditions:
		// [*] The Steam client isn't running. A running Steam client is required to provide implementations of the various SteamworksNet interfaces.
		...
		if (!m_bInitialized) {
			Debug.LogError("[SteamworksNet.NET] SteamAPI_Init() failed. Refer to Valve's documentation or the comment above this line for more information.", this);
